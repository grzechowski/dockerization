...
${co}Available commands${cn}:
   ...
   ${cg}link${cn}  project  path-to-repo                # link your project with project repository DEV_REPO
   ${cg}unlink${cn}  project                            # unlink your project with project repository DEV_REPO
...
function console_repo_link {
  ...
  dev_repo_link="$DOCKER_CONSOLE_DOCKERS_DIR_PATH"/"$1"/DEV_REPO
  ...
  sed -i "s/DEV_REPO/$SED_VAR/g" "$DOCKER_CONSOLE_DOCKERS_DIR_PATH"/"$1"/docker-compose.yml
...
do
  case "$1" in
    ...
    link)
      p=$(console_project_exist $2)
      console_repo_link $p $3
      console_loop_escape=1
      ;;
    unlink)
      p=$(console_project_exist $2)
      console_repo_unlink $p
      console_loop_escape=1
      ;;
    proxy)
      p=$(console_project_exist $2)
